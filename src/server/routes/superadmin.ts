...
// POST /api/superadmin/tenants/:id/custom-plan - Create & Assign a Custom Tier Plan with custom features & pricing
router.post('/tenants/:id/custom-plan', async (req, res) => {
  try {
    const { id } = req.params;
    const {
      planName,
    ...
    // Calculate 1 month renewal date
    const nextPeriodEnd = new Date();
    nextPeriodEnd.setMonth(nextPeriodEnd.getMonth() + 1);

    // Update Tenant with custom plan & billing provider info
    const updatedTenant = await prismaRaw.tenant.update({
      where: { id },
      data: {
