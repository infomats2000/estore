  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl text-white shadow-xl flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-xl font-black tracking-tight mt-1">Automatic Customer-Specific Price Differentiation</h2>
            <p className="text-xs text-slate-400">Same product SKU automatically renders different contract prices per reseller account</p>
          </div>
        </div>
      ...
      <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 flex flex-wrap items-center gap-2 font-mono text-[11px]">
        <span className="text-slate-400 font-bold uppercase text-[10px] mr-2">Supported Pricing Dimensions:</span>
        {[
          ...
          <span key={dim} className="px-2.5 py-1 bg-slate-900 text-purple-300 rounded-lg border border-slate-800 font-bold">
            {dim}
          </span>
        ))}
      </div>
      ...
      <div className="bg-slate-900 p-2 rounded-2xl border border-slate-800 flex flex-wrap gap-2">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'simulator' ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'rules' ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'brand_category' ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'deals' ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
          }`}
        >
      ...
      {activeTab === 'simulator' && (
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 p-5 rounded-3xl border border-slate-800 flex flex-wrap justify-between items-center gap-4">
            <div>
              <span className="text-[10px] font-mono font-bold text-purple-400 uppercase tracking-widest bg-purple-950 px-2.5 py-0.5 rounded border border-purple-800">
                ACTIVE HARDWARE PRODUCT SKU
              </span>
              <h3 className="text-lg font-black text-slate-100 mt-1">{selectedProduct.name}</h3>
              <span className="text-xs font-mono text-slate-400">Base Retail MSRP: <strong className="text-slate-200">${selectedProduct.price.toFixed(2)} AUD</strong></span>
            </div>

            <div className="flex items-center gap-3">
              <label className="text-xs font-mono text-slate-400 font-bold">Simulate Order Qty:</label>
              <input
                type="number"
                value={simQty}
                onChange={e => setSimQty(e.target.value)}
                className="bg-slate-950 border border-slate-800 px-3 py-1.5 rounded-xl text-xs font-mono font-bold text-emerald-400 w-24"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-mono">
            {SAMPLE_CUSTOMERS_PRICING_DEMO.map(cust => {
              ...
              return (
                <div key={cust.id} className="bg-slate-900 p-6 rounded-3xl border border-slate-800 space-y-4 shadow-xl relative overflow-hidden">
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="px-2.5 py-0.5 text-[10px] font-bold uppercase bg-blue-950 text-blue-300 rounded border border-blue-800">
                        {cust.tier}
                      </span>
                      <h4 className="font-bold text-slate-100 text-sm mt-2">{cust.name}</h4>
                    </div>
                  </div>

                  {/* Calculated Dynamic Price Highlight */}
                  <div className="bg-slate-950 p-5 rounded-2xl border border-purple-500/40 space-y-1 text-center">
                    <span className="text-[10px] text-slate-400 uppercase font-bold">Automated Customer Price</span>
                    <div className="text-3xl font-black text-emerald-400">${res.finalPrice.toFixed(2)} AUD</div>
                    <span className="text-[11px] text-purple-300 font-bold block">{res.effectiveDiscountPercent}% Off MSRP (Save ${res.savingsAmount.toFixed(2)})</span>
                  </div>

                  <div className="text-xs text-slate-300 space-y-1 pt-2 border-t border-slate-800">
                    <div>Applied Rule: <strong className="text-purple-400">{res.appliedRuleType}</strong></div>
                    <div className="text-[11px] text-slate-400 mt-1">{res.appliedRuleDescription}</div>
                  </div>
                </div>
              );
            })}
      ...
      {activeTab === 'rules' && (
        <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 space-y-4 font-mono text-xs">
          <h3 className="text-sm font-black uppercase text-slate-100">Customer-Specific Matrix Price Rules</h3>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                {rules.map(r => (
                  <tr key={r.id}>
                    <td className="p-3">
                      <span className="px-2 py-0.5 bg-purple-950 text-purple-300 font-bold rounded border border-purple-800 text-[10px]">{r.id}</span>
                    </td>
                    <td className="p-3 font-bold text-slate-100">{r.customerName}</td>
                    <td className="p-3 text-blue-300 font-bold">{r.tierType}</td>
                    <td className="p-3 text-slate-400">{r.productName || r.brand || r.category || 'All Products'}</td>
      ...
      {showAddRuleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 animate-fade-in font-mono text-xs">
          <div className="bg-slate-900 w-full max-w-md rounded-3xl border border-slate-800 p-6 space-y-4 text-white">
            <h3 className="font-black text-sm uppercase text-slate-100 border-b border-slate-800 pb-3">Create Customer Price Rule</h3>

            <div className="space-y-3">
              <div>
                <label className="text-slate-400 uppercase text-[10px] font-bold block mb-1">Customer Account</label>
                <select
                  value={newCustId}
                  onChange={e => {
              ...
              <div>
                <label className="text-slate-400 uppercase text-[10px] font-bold block mb-1">Pricing Dimension Tier</label>
                <select
                  value={newTierType}
              ...
              <div>
                <label className="text-slate-400 uppercase text-[10px] font-bold block mb-1">Contract Override Price ($ AUD)</label>
                <input
                  type="number"
                  value={newPrice}
