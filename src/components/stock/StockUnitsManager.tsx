  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'In Stock', value: stats.inStock, color: 'border-emerald-300 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-950/40', textColor: 'text-emerald-700 dark:text-emerald-400' },
          { label: 'Sold', value: stats.sold, color: 'border-slate-300 bg-slate-50 dark:border-slate-800 dark:bg-slate-900', textColor: 'text-slate-700 dark:text-slate-300' },
          { label: 'In Repair', value: stats.inRepair, color: 'border-amber-300 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/40', textColor: 'text-amber-700 dark:text-amber-400' },
          { label: 'Total Units', value: stats.total, color: 'border-blue-300 bg-blue-50 dark:border-blue-800 dark:bg-blue-950/40', textColor: 'text-blue-700 dark:text-blue-400' },
        ].map(s => (
        ...
        <div className="flex gap-1">
          {['All', ...ALL_STATUSES].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-colors ${statusFilter === s ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900' : 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300'}`}>
              {s}
            </button>
          ))}
        </div>
        <button onClick={exportCSV} className="flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 px-3 py-2 rounded-lg text-xs font-bold">
