      ...
      <div className="p-5 space-y-5" id="pos-operational-report">
        ...
        {report && <>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">{[
            ['Sales', `$${report.sales.total.toFixed(2)}`], ['GST', `$${report.sales.tax.toFixed(2)}`], ['Refunds', `$${report.refunds.total.toFixed(2)}`], ['Drawer variance', `$${report.shifts.variance.toFixed(2)}`], ['Lay-by outstanding', `$${report.laybys.outstanding.toFixed(2)}`],
          ...
          <div className="text-[10px] text-neutral-500">{report.sales.count} paid sales · {report.refunds.count} returns · {report.shifts.count} shifts · {report.laybys.created} lay-bys created</div>
        </>}
      </div>
    </div>
  </div>;
