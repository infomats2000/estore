  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl text-white shadow-xl flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-xl font-black tracking-tight mt-1">RFQs, Multi-Currency Bids &amp; Landed Cost Allocation</h2>
            <p className="text-xs text-slate-400">Side-by-Side Supplier Quotations, Container Receiving &amp; Manager Approvals</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 flex flex-wrap items-center gap-2 font-mono text-[11px]">
        <span className="text-slate-400 font-bold uppercase text-[10px] mr-2">Purchasing Features:</span>
        {[
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              activeTab === 'rfqs' ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              activeTab === 'comparison' ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              activeTab === 'approvals' ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              activeTab === 'landed_costs' ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 font-mono text-xs">
        ...
        <div className="space-y-3">
          <h3 className="text-xs font-black uppercase tracking-wider text-slate-400 font-sans">Active Global RFQs</h3>
          <div className="space-y-3">
            {rfqs.map(rfq => (
              <div
                key={rfq.id}
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="px-2.5 py-0.5 text-[10px] font-bold bg-purple-950 text-purple-300 rounded border border-purple-800">{rfq.id}</span>
                  <span className="px-2 py-0.5 text-[10px] font-bold bg-blue-950 text-blue-300 rounded border border-blue-800">{rfq.status}</span>
                </div>

                <h4 className="font-bold text-sm text-slate-100 mt-2 font-sans">{rfq.title}</h4>
                <div className="text-[11px] text-slate-400 mt-1 flex justify-between">
                  <span>Qty: <strong>{rfq.requestedQty} Units</strong></span>
                  <strong className="text-emerald-400">${rfq.poTotalAUD.toLocaleString()} AUD</strong>
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 space-y-6">
              <div className="flex flex-wrap items-center justify-between border-b border-slate-800 pb-4 gap-3">
                <div>
                  ...
                  <h3 className="text-lg font-black text-slate-100 mt-1 font-sans">{selectedRFQ.title}</h3>
                  <span className="text-xs text-slate-400">Target Item: {selectedRFQ.targetSKU}</span>
                </div>
              ...
              <div className="space-y-2">
                <h4 className="font-bold uppercase text-slate-400 text-[10px] font-sans">Multiple Supplier Quotations &amp; FX Rate Matrix</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {selectedRFQ.quotes.map(q => (
                    <div
                      key={q.supplierId}
                      className={`p-4 rounded-2xl border ${
                        selectedRFQ.winningSupplierId === q.supplierId ? 'bg-slate-950 border-purple-500 shadow-lg' : 'bg-slate-950/60 border-slate-800'
                      }`}
                    >
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] font-bold uppercase text-purple-400 bg-purple-950 px-2 py-0.5 rounded border border-purple-800">
                          {q.currency} Bid
                        </span>
                        <span className="text-xs font-bold text-amber-400">Score: {q.qualityScore}/100</span>
                      </div>

                      <h5 className="font-bold text-slate-100 text-xs mt-2 font-sans">{q.supplierName}</h5>
                      <div className="text-[11px] text-slate-300 space-y-1 mt-2">
                        ...
                        <div>Converted AUD: <strong className="text-emerald-400">${q.unitPriceAUD.toFixed(2)} AUD</strong></div>
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
                <h4 className="font-bold uppercase text-purple-400 text-[10px] font-sans">Landed Cost Allocation Breakdown (Sea Freight, Duty &amp; Customs)</h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-[11px]">
                  <div>Sea Freight: <strong className="text-slate-200">${selectedRFQ.freightCostAUD.toFixed(2)} AUD</strong></div>
                  <div>Import Duty Tax: <strong className="text-purple-300">${selectedRFQ.dutyTaxAUD.toFixed(2)} AUD</strong></div>
                  <div>Customs Charges: <strong className="text-blue-300">${selectedRFQ.customsChargesAUD.toFixed(2)} AUD</strong></div>
                  <div>Allocated Cost / Unit: <strong className="text-emerald-400">+${selectedRFQ.allocatedLandedCostPerUnit.toFixed(2)} AUD</strong></div>
                </div>
              </div>
            </div>
          ) : (
            <div className="bg-slate-900 p-12 rounded-3xl border border-slate-800 text-center space-y-3 text-slate-400 font-sans">
              <Globe className="w-10 h-10 mx-auto text-purple-400 opacity-60" />
              <h4 className="font-bold text-sm text-slate-200">Select a Procurement RFQ</h4>
