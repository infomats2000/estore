    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
          <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400">CONNECTED CHANNELS</span>
          <div className="text-2xl font-black text-slate-100 mt-1">4 Marketplaces</div>
          <span className="text-[11px] text-slate-400 mt-1 block font-mono">Amazon, eBay, Facebook, Google</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
          <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400">SINGLE INVENTORY POOL</span>
          <div className="text-2xl font-black text-emerald-400 mt-1">1 Central Stock</div>
          <span className="text-[11px] font-slate-400 mt-1 block">Zero Overselling Guarantee</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
          <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400">ACTIVE LISTINGS</span>
          <div className="text-2xl font-black text-blue-400 mt-1">{listings.length} Syncing Items</div>
          <span className="text-[11px] text-slate-400 mt-1 block font-mono">15-Min Auto Refresh</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
          <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400">API RATE LIMIT ALLOWANCE</span>
          <div className="text-2xl font-black text-purple-400 mt-1">4,920 / 5,000</div>
          <span className="text-[11px] text-slate-400 mt-1 block font-mono">98% Health Score</span>
        </div>
      </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              activeTab === 'accounts' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              activeTab === 'listings' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              activeTab === 'feeds' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
      ...
      {activeTab === 'accounts' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {accounts.map(acc => (
            <div key={acc.id} className="bg-slate-900 p-5 rounded-3xl border border-slate-800 space-y-3">
              <div className="flex items-center justify-between">
                <span className="px-2.5 py-0.5 text-[10px] font-mono font-bold uppercase bg-blue-950 text-blue-300 rounded border border-blue-800">{acc.channel}</span>
              ...
              <div>
                <h4 className="font-bold text-sm text-slate-100">{acc.storeName}</h4>
                <span className="text-[11px] font-mono text-slate-400">Seller ID: {acc.sellerId}</span>
              </div>

              <div className="bg-slate-950 p-2.5 rounded-xl border border-slate-800 font-mono text-[11px] text-slate-400 space-y-1">
                <div>Marketplace: <strong className="text-slate-200">{acc.marketplace}</strong></div>
                <div>Sync Frequency: <strong className="text-blue-300">Every {acc.syncFrequencyMinutes} mins</strong></div>
                <div>Token Status: <strong className="text-emerald-400">Encrypted OAuth Active</strong></div>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-900 p-3 rounded-2xl border border-slate-800">
              ...
              {['All', 'Amazon', 'eBay', 'Facebook', 'GoogleShopping'].map(ch => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                    channelFilter === ch ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  {ch}
                </button>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="bg-slate-950 border border-slate-800 pl-9 pr-4 py-1.5 text-xs rounded-xl text-white w-64"
              />
            </div>
          </div>
            ...
            <table className="w-full text-left font-mono text-xs">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                {filteredListings.map(lst => (
                  <tr key={lst.id} className="hover:bg-slate-900/50">
                    <td className="p-3">
                      <span className="px-2 py-0.5 bg-blue-950 text-blue-300 font-bold rounded border border-blue-800 text-[10px]">{lst.channel}</span>
                    </td>
                    <td className="p-3">
                      <div className="font-bold text-slate-100 line-clamp-1">{lst.title}</div>
                      <span className="text-[10px] text-slate-400">SKU: {lst.sku}</span>
                    </td>
                    <td className="p-3 text-slate-300">{lst.externalListingId}</td>
                    <td className="p-3 font-bold text-emerald-400">${lst.price.toFixed(2)}</td>
                    <td className="p-3">
                      <span className="px-2 py-0.5 bg-slate-950 text-slate-200 font-bold rounded border border-slate-800">{lst.quantity} Units</span>
                    </td>
                    <td className="p-3">
                      <span className="px-2 py-0.5 bg-emerald-950 text-emerald-400 font-bold rounded border border-emerald-800 text-[10px]">{lst.status}</span>
                    </td>
                    <td className="p-3 text-right">
                      <a href={lst.listingUrl} target="_blank" rel="noreferrer" className="text-blue-400 hover:underline flex items-center justify-end gap-1">
      ...
      {activeTab === 'feeds' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-sm font-black uppercase text-slate-100 flex items-center gap-2">
                <Globe className="w-4 h-4 text-blue-400" /> Google Shopping Merchant Center Feed
              </h3>
              <span className="text-[10px] font-mono text-emerald-400 font-bold bg-emerald-950 px-2 py-1 rounded border border-emerald-800">RSS 2.0 XML</span>
            </div>
          ...
          <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-sm font-black uppercase text-slate-100 flex items-center gap-2">
                <Share2 className="w-4 h-4 text-purple-400" /> Meta Commerce (Facebook &amp; Instagram Shop)
              </h3>
              <span className="text-[10px] font-mono text-purple-300 font-bold bg-purple-950 px-2 py-1 rounded border border-purple-800">CSV Catalog</span>
            </div>
