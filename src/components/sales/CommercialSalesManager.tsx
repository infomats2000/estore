  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl text-white shadow-xl flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-xl font-black tracking-tight mt-1">Quotations, Blanket Orders, Contract Pricing &amp; Drop Shipping</h2>
            <p className="text-xs text-slate-400">Enterprise Commercial Sales Management for IT Wholesalers &amp; Distributors</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 flex flex-wrap items-center gap-2 font-mono text-[11px]">
        <span className="text-slate-400 font-bold uppercase text-[10px] mr-2">Commercial Capabilities:</span>
        {['Sales Quotations', 'Proforma Invoices', 'Tax Invoices', 'Credit Sales (Net 30/60)', 'Cash Sales', 'Contract Pricing', 'Blanket Orders', 'Standing Orders', 'Partial Shipments', 'Split Shipments', 'Backorders', 'Drop Shipping'].map(cap => (
          <span key={cap} className="px-2.5 py-1 bg-slate-900 text-emerald-300 rounded-lg border border-slate-800 font-bold">
            {cap}
          </span>
        ))}
      </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              activeTab === 'all' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              activeTab === 'quotes' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              activeTab === 'blanket_standing' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              activeTab === 'fulfillment' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <h3 className="text-xs font-black uppercase tracking-wider text-slate-400">Document Records</h3>
          <div className="space-y-3">
            {filteredOrders.map(ord => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="px-2.5 py-0.5 text-[10px] font-mono font-bold bg-emerald-950 text-emerald-300 rounded border border-emerald-800">{ord.id}</span>
                  <span className="px-2 py-0.5 text-[10px] font-mono font-bold bg-blue-950 text-blue-300 rounded border border-blue-800">{ord.orderType}</span>
                </div>

                <h4 className="font-bold text-sm text-slate-100 mt-2">{ord.customerName}</h4>
                <div className="text-[11px] font-mono text-slate-400 mt-1 flex justify-between">
                  <span>{ord.paymentCategory}</span>
                  <strong className="text-emerald-400">${ord.total.toLocaleString()} AUD</strong>
                </div>
              </div>
            ))}
          </div>
          ...
          {selectedOrder ? (
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 space-y-6">
              <div className="flex flex-wrap items-center justify-between border-b border-slate-800 pb-4 gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono font-bold text-emerald-400 uppercase tracking-widest bg-emerald-950 px-2 py-0.5 rounded border border-emerald-800">
                  ...
                  <h3 className="text-lg font-black text-slate-100 mt-1">{selectedOrder.customerName}</h3>
                  <span className="text-xs font-mono text-slate-400">{selectedOrder.companyName}</span>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 font-mono text-xs">
                <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800">
                  <span className="text-[10px] text-slate-400 uppercase block">Payment &amp; Trade Category</span>
                  <span className="font-bold text-amber-400 mt-1 block">{selectedOrder.paymentCategory}</span>
                </div>

                <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800">
                  <span className="text-[10px] text-slate-400 uppercase block">Contract Pricing Matrix</span>
                  <span className="font-bold text-purple-300 mt-1 block">{selectedOrder.contractPriceTier || 'Standard Commercial List'}</span>
                </div>

                <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800">
                  <span className="text-[10px] text-slate-400 uppercase block">Fulfillment Strategy</span>
                  <span className="font-bold text-emerald-400 mt-1 block">{selectedOrder.fulfillmentMode}</span>
                </div>
              </div>

              {/* Special Order Badges */}
              {selectedOrder.blanketCommitmentUnits && (
                <div className="bg-purple-950/40 p-4 rounded-2xl border border-purple-800/60 font-mono text-xs text-purple-300 flex justify-between items-center">
                  <span>BLANKET ORDER COMMITMENT: <strong>{selectedOrder.blanketCommitmentUnits} Units Total</strong></span>
                  <span className="px-3 py-1 bg-purple-900 font-bold rounded-lg border border-purple-700">
              ...
              {selectedOrder.dropShipClientAddress && (
                <div className="bg-blue-950/40 p-4 rounded-2xl border border-blue-800/60 font-mono text-xs text-blue-300">
                  <span className="font-bold uppercase text-[10px] block text-blue-400">VENDOR DROP SHIP CLIENT DELIVERY ADDRESS:</span>
              ...
              <div className="space-y-2 font-mono text-xs">
                <h4 className="font-bold uppercase text-slate-400 text-[10px]">Order Line Items &amp; Partial Shipment Balances</h4>
                    ...
                    <tbody className="divide-y divide-slate-800/60 text-slate-300">
                      {selectedOrder.items.map((item, idx) => (
                        <tr key={idx}>
                          ...
                          <td className="p-3 text-right text-emerald-400 font-bold">{item.shippedQty}</td>
                          <td className="p-3 text-right text-amber-400 font-bold">{item.backorderQty}</td>
                          <td className="p-3 text-right">${item.unitPrice.toFixed(2)}</td>
                          <td className="p-3 text-right font-bold text-slate-100">${item.totalPrice.toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {/* Total Summary */}
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex justify-between items-center font-mono text-xs">
                <span className="text-slate-400">Commercial Tax Invoice Total (incl. 10% AU GST):</span>
                <span className="text-2xl font-black text-emerald-400">${selectedOrder.total.toLocaleString()} AUD</span>
              </div>
            </div>
          ) : (
            <div className="bg-slate-900 p-12 rounded-3xl border border-slate-800 text-center space-y-3 text-slate-400">
              <FileText className="w-10 h-10 mx-auto text-emerald-400 opacity-60" />
              <h4 className="font-bold text-sm text-slate-200">Select a Commercial Document Record</h4>
