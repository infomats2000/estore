    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
          <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400">ACTIVE STORE BRANCHES</span>
          <div className="text-2xl font-black text-slate-100 mt-1">{stores.length} Retail Stores</div>
          <span className="text-[11px] text-slate-400 mt-1 block font-mono">Australia, New Zealand &amp; USA</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
          <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400">ACTIVE INTER-STORE TRANSFERS</span>
          <div className="text-2xl font-black text-amber-400 mt-1">{transfers.filter(t => t.status === 'In Transit').length} In Transit</div>
          <span className="text-[11px] text-slate-400 mt-1 block">Real-time Stock Relocation</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
          <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400">SUPPORTED CURRENCIES</span>
          <div className="text-2xl font-black text-blue-400 mt-1">5 Global FX</div>
          <span className="text-[11px] text-slate-400 mt-1 block font-mono">AUD, NZD, USD, GBP, EUR</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
          <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400">REGIONAL TAX COMPLIANCE</span>
          <div className="text-xl font-black text-emerald-400 mt-1 flex items-center gap-1.5">
            <ShieldCheck className="w-5 h-5 text-emerald-400" />
            <span>10% GST / 15% NZ GST</span>
          </div>
          <span className="text-[11px] text-slate-400 mt-1 block font-mono">Automated Regional Tax Calculations</span>
        </div>
      </div>
      ...
      <div className="bg-slate-900 p-2 rounded-2xl border border-slate-800 flex flex-wrap gap-2">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'branches' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'transfers' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'regional_pricing' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'tax_currencies' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
          }`}
        >
      ...
      {activeTab === 'branches' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black uppercase text-slate-200">Store Branches &amp; Regional Outlets</h3>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {stores.map(store => (
              <div key={store.id} className="bg-slate-900 p-5 rounded-3xl border border-slate-800 space-y-3">
                <div className="flex justify-between items-start">
                  <div>
                    <span className="px-2.5 py-0.5 text-[10px] font-mono font-bold uppercase bg-blue-950 text-blue-300 rounded border border-blue-800">{store.code}</span>
                    <h4 className="font-bold text-sm text-slate-100 mt-1">{store.storeName}</h4>
                    <span className="text-[11px] font-mono text-slate-400">{store.region}</span>
                  </div>
                  <span className="text-xs font-mono font-bold text-emerald-400 bg-emerald-950/60 px-2 py-1 rounded-lg border border-emerald-800">
                    Tax: {store.localTaxRatePercent}%
                  </span>
                </div>

                <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800/80 font-mono text-xs space-y-1 text-slate-400">
                  <div>Currency: <strong className="text-slate-200">{store.currencySymbol} ({store.currencyCode}) @ {store.currencyRateToAUD}x AUD</strong></div>
                  <div>Assigned WH: <strong className="text-blue-300">{store.defaultWarehouseId}</strong></div>
                  <div>Address: <strong className="text-slate-300">{store.address}</strong></div>
                  <div>Phone: <strong className="text-slate-300">{store.phone}</strong></div>
                </div>
              </div>
            ))}
          </div>
      ...
      {activeTab === 'transfers' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black uppercase text-slate-200">Inter-Branch Inventory Transfers</h3>
            <button
          ...
          <div className="space-y-3">
            {transfers.map(trf => (
              <div key={trf.id} className="bg-slate-900 p-5 rounded-3xl border border-slate-800 flex flex-wrap items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-3">
                    <span className="px-2.5 py-1 bg-amber-950 text-amber-300 text-xs font-mono font-bold rounded-lg border border-amber-800">{trf.transferNumber}</span>
                    <span className="text-xs font-bold text-slate-200">{trf.sourceStoreName} &rarr; {trf.destinationStoreName}</span>
                  </div>
                ...
                <div className="flex items-center gap-3">
                  <span className={`px-2.5 py-1 text-xs font-mono font-bold rounded-lg border ${
                    trf.status === 'Completed' ? 'bg-emerald-950 text-emerald-400 border-emerald-800' : 'bg-amber-950 text-amber-400 border-amber-800'
                  }`}>
                    {trf.status}
                  </span>
      ...
      {activeTab === 'regional_pricing' && (
        <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 space-y-4">
          <h3 className="text-sm font-black uppercase text-slate-100">Regional Price Overrides Matrix</h3>
          <div className="space-y-3 font-mono text-xs">
            {products.slice(0, 5).map(prod => (
              <div key={prod.id} className="bg-slate-950 p-4 rounded-2xl border border-slate-800/80 space-y-2">
                <div className="flex justify-between items-center">
                  <span className="font-bold text-slate-100">{prod.name}</span>
                  <span className="text-slate-400">Base AUD Price: <strong className="text-emerald-400">${prod.price.toFixed(2)} AUD</strong></span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[11px]">
                  {stores.map(store => {
                    ...
                    return (
                      <div key={store.id} className="bg-slate-900 p-2.5 rounded-xl border border-slate-800">
                        <span className="text-slate-400 block text-[10px] uppercase font-bold">{store.code} ({store.currencyCode})</span>
                        <strong className="text-blue-300">{regional.formatted}</strong>
                      </div>
                    );
                  })}
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 space-y-4">
            <h3 className="text-sm font-black uppercase text-slate-100 flex items-center gap-2">
              <Percent className="w-4 h-4 text-purple-400" /> Regional Local Tax Rules
            </h3>
            <div className="space-y-2 font-mono text-xs">
              <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 flex justify-between">
                <span className="text-slate-300">Australia (NSW, VIC, QLD):</span>
                <strong className="text-purple-400">10.0% ATO GST</strong>
              </div>
              <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 flex justify-between">
                <span className="text-slate-300">New Zealand (Auckland):</span>
                <strong className="text-purple-400">15.0% IRD GST</strong>
              </div>
              <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 flex justify-between">
                <span className="text-slate-300">United States (California):</span>
                <strong className="text-purple-400">8.25% Sales Tax</strong>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 space-y-4">
            <h3 className="text-sm font-black uppercase text-slate-100 flex items-center gap-2">
              <Globe className="w-4 h-4 text-blue-400" /> Live FX Multi-Currency Exchange Rates
            </h3>
            <div className="space-y-2 font-mono text-xs">
              <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 flex justify-between">
                <span className="text-slate-300">1.00 AUD &rarr; AUD (Base Currency)</span>
                <strong className="text-blue-400">1.00x</strong>
              </div>
              <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 flex justify-between">
                <span className="text-slate-300">1.00 AUD &rarr; NZD (New Zealand Dollar)</span>
                <strong className="text-blue-400">1.08x NZD</strong>
              </div>
              <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 flex justify-between">
                <span className="text-slate-300">1.00 AUD &rarr; USD (US Dollar)</span>
                <strong className="text-blue-400">0.65x USD</strong>
              </div>
            </div>
          </div>
        </div>
      ...
      {showAddStoreModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 animate-fade-in">
          <div className="bg-slate-900 w-full max-w-md rounded-3xl border border-slate-800 p-6 space-y-4 text-white">
            <h3 className="font-black text-sm uppercase text-slate-100 border-b border-slate-800 pb-3">Add Store Branch</h3>
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 px-3 py-2 text-xs rounded-xl text-white uppercase font-mono"
              />
            </div>
      ...
      {showTransferModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 animate-fade-in">
          <div className="bg-slate-900 w-full max-w-md rounded-3xl border border-slate-800 p-6 space-y-4 text-white">
            <h3 className="font-black text-sm uppercase text-slate-100 border-b border-slate-800 pb-3">Create Inter-Branch Stock Transfer</h3>
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 px-3 py-2 text-xs rounded-xl text-emerald-400 font-mono"
              />
            </div>
